...
//function to compare n no. of characters
extern int mv_strncmp(char* str1, char* str2, long long user_NRcharacters)
{
	//code
	if(user_NRcharacters <= 0)
	{
		fprintf(stderr, "ERROE: user_NRcharacters have an INVALID value\n");
		return(0);
	}
	//step1: take length of str1
	long long str1Length = mv_strlen(str1);
	...
	while(1)
	{   
		if(index >= user_NRcharacters) //check for nr_characters reached
		{
			break;
		}
...
//function to find the  first position of given character

extern char* mv_strchr(char* str1, char ch)
{
	//code
	long long strLength = mv_strlen(str1);
...
//function to find the  first position of given character from the end

extern char* mv_strrchr(char* str1, char ch)
{
	//code
	long long strLength = mv_strlen(str1);

	//for(long long le = 0; le < strLength; ++le) forward loop
	for(long long le = strLength - 1; le >= 0; --le) //Reverse loop
	{
		if(str1[le] == ch)
		{
			return(&str1[le]);
...
//function to find one string in another string
extern char* mv_strstr(char* str1, char* str2)
{
    if (!str1 || !str2 || str2[0] == '\0') return str1;
